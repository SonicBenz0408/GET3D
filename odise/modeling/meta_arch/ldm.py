...
def build_ldm_from_cfg(cfg_name) -> _LatentDiffusion:
    ...
    if cfg_name.startswith("v1"):
        url_prefix = "pretrained_model/"  # noqa
    ...
    config = OmegaConf.load(url_prefix + cfg_name)
...
class LatentDiffusion(nn.Module):
    ...
    def load_pretrain(self, config, ckpt="pretrained_model/sd-v1-4.ckpt", verbose=True):
        ...
        sd = pl_sd["state_dict"]
        m, u = self.ldm.load_state_dict(sd, strict=False)
