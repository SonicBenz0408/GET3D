...
def training_loop(
    ...
    # geo_diffusion_model = GaussianDiffusion1D_cond(geo_Unet, seq_length=G_kwargs.z_dim, timesteps=1000, objective='pred_noise').to(device)
    tex_Unet = Unet1D_cond(dim=diff_ch, dim_mults=(1, 2, 4, 8), channels=1, cond_ch=1, resnet_block_groups=2).to(device)
    tex_diffusion_model = GaussianDiffusion1D_cond(tex_Unet, seq_length=G_kwargs.z_dim, timesteps=1000, objective='pred_noise').to(device)
    ...
    if rank == 0:
        ...
        grid_z = torch.randn([n_shape, G_kwargs.z_dim], device=device).split(1)  # random code for geometry
        grid_tex_z = torch.randn([n_shape, G_kwargs.z_dim], device=device).unsqueeze(1).repeat(1, tex_diffusion_model.model.cond_ch, 1).split(1)  # random code for texture
        grid_c = torch.ones(n_shape, device=device).split(1)
        grid_cond = torch.zeros((n_shape, clip_model.visual.output_dim), device=device).unsqueeze(1).split(1)
    ...
    with tqdm(initial=step, total=train_num_steps, disable = not accelerator.is_main_process) as pbar:
        ...
        while step < train_num_steps:
            ...
            # training data
            preprocess_img = preprocess_img.detach().to(device)

            clip_c = clip_model.encode_image(preprocess_img)

            cond = clip_c.unsqueeze(1)

            total_loss = 0.

            with accelerator.autocast():
                # geo_loss = geo_diffusion_model(gen_geo_z.detach(), cond)
                tex_loss = tex_diffusion_model(gen_tex_z.unsqueeze(1).detach(), cond)
                loss = tex_loss
            ...
            if accelerator.is_main_process:
                ...
                if step != 0 and step % image_snapshot_ticks == 0:
                    ...
                    save_visualization_with_cond(
                        G_ema, None, ema_tex_diffusion_model.ema_model, grid_cond, grid_z, grid_c, run_dir, 0, grid_size, step,
                        save_all=False,
                        grid_tex_z=grid_tex_z
                    )

            if step % network_snapshot_ticks == 0:
                #snapshot_data = dict(geo_diff=geo_diffusion_model, ema_geo_diff=ema_geo_diffusion_model)
                snapshot_data = dict(tex_diff=tex_diffusion_model, ema_tex_diff=ema_tex_diffusion_model)
                snapshot_pkl = os.path.join(run_dir, f'diffusion-network-snapshot-{step}.pkl')
                #all_model_dict = {'geo_diff': snapshot_data['geo_diff'].state_dict(), 'ema_geo_diff': snapshot_data['ema_geo_diff'].state_dict()}
                all_model_dict = {'tex_diff': snapshot_data['tex_diff'].state_dict(), 'ema_tex_diff': snapshot_data['ema_tex_diff'].state_dict()}
