...
@persistence.persistent_class
class Discriminator_new(torch.nn.Module):
    def __init__(
            self,
            c_dim,  # Conditioning label (C) dimensionality.
            img_resolution,  # Input resolution.
        ...
        if self.c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=self.c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, device=device, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=1, resolution=4, device=device, **epilogue_kwargs, **common_kwargs)
...
class Classifier(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, alpha=1.0, **block_kwargs):
        # Encoding camera condition first
        # if self.add_camera_cond:
        #     if self.conditional_dim == 0:
        #         c = self.pos_enc_angle(c)
