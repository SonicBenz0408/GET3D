...
def inference_diff(
    ...
    grid_z = torch.randn([n_shape, G_kwargs.w_dim], device=device).split(1)  # random code for geometry
    grid_tex_z = torch.randn([n_shape, G_kwargs.w_dim], device=device).split(1)  # random code for texture
