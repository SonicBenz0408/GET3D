...
class Unet1D_cond(nn.Module):
    def __init__(
        ...
        input_channels = channels + cond_ch
...
class GaussianDiffusion1D(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, z=None):
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, clip_denoised = True, z = None):
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, z = None):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, seq_length), z)
...
class GaussianDiffusion1D_cond(nn.Module):
    def __init__(
        self,
        model,
        *,
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond, shape, z = None):
        batch, device = shape[0], self.betas.device
    ...
    @torch.no_grad()
    def ddim_sample(self, cond, shape, clip_denoised = True, z = None):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
    ...
    @torch.no_grad()
    def sample(self, cond, batch_size = 16, z = None):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(cond, (batch_size, channels, seq_length), z)
